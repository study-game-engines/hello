...
namespace BVH {
    ...
    RayTraversalResult MeshAnyHit(uint32_t roodNodeIndex, glm::vec3 rayOrigin, glm::vec3 rayDir, float maxDistance, glm::mat4 meshWorldTransform) {
        uint32_t stack[MAX_BVH_STACK_SIZE];
        size_t currentStackSize = 0;

        // Calculate world ray in mesh space
        glm::mat4 inverseTransform = glm::inverse(meshWorldTransform);
            ...
            if (node.primitiveCount > 0) {
                for (int i = 0; i < node.primitiveCount; i++) {
                    ...
                    if (IntersectTri(localOrigin, localDir, localMinDistance, localMaxDistance, p0, e1, e2, normal, localT)) {
                        ...
                        glm::vec3 hitPositionWorld = meshWorldTransform * glm::vec4(hitPositionLocal, 1.0f);
                        ...
                        rayResult.primitiveTransform = meshWorldTransform;
        ...
        while (currentStackSize != 0) {
            ...
            if (node.primitiveCount > 0) {
                ...
                for (int i = 0; i < node.primitiveCount; i++) {
                    ...
                    RayTraversalResult localRayResult = MeshAnyHit(instance.bvhRootNodeIndex, rayOrigin, rayDir, maxDistance, instance.worldTransform);
    ...
    RayTraversalResult MeshClosestHit(uint32_t roodNodeIndex, glm::vec3 rayOrigin, glm::vec3 rayDir, float maxDistance, glm::mat4 meshWorldTransform) {
        uint32_t stack[MAX_BVH_STACK_SIZE];
        size_t currentStackSize = 0;
                ...
                for (int i = 0; i < node.primitiveCount; i++) {
                    ...
                    if (IntersectTri(localOrigin, localDir, rayData.minDistance, rayData.maxDistance, p0, e1, e2, normal, localT)) {
                        ...
                        if (distanceToHit < closestRayResult.distanceToHit) {
                            ...
                            closestRayResult.primitiveTransform = meshWorldTransform;
    ...
    RayTraversalResult ClosestHit(uint64_t sceneBvhId, glm::vec3 rayOrigin, glm::vec3 rayDir, float maxDistance) {
        ...
        while (currentStackSize != 0) {
            ...
            if (node.primitiveCount > 0) {
                ...
                // It is, so now iterate the instances and check for a ray hit on those
                for (int i = 0; i < node.primitiveCount; i++) {
                    const GpuPrimitiveInstance& entityInstance = instances[node.firstChildOrPrimitive + i];
                    RayTraversalResult localRayResult = MeshClosestHit(entityInstance.bvhRootNodeIndex, rayOrigin, rayDir, rayResult.distanceToHit, entityInstance.worldTransform);
